<script lang="ts">
	...
	import { signIn } from '$lib/features/auth/authService';
	...
	// Derived state
	let notEmpty = $derived(fields.username && fields.password);
	...
	async function _signIn(event: Event) {
		...
		try {
			...
			if (response.success) {
				...
				formState.errorMessage =
					response.errors.length > 0 ? response.errors[0].message : 'Sign in failed';
			}
		} catch (error) {
...
>
	<section class="flex flex-col items-center">
		<h1 class="flex text-lg font-bold">Welcome Back!</h1>
		<h3 class="flex text-center text-xs">Sign in to your account</h3>
	</section>
